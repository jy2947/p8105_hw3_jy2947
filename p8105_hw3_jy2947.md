...
This dataset contains some information of weather at different locations in the US. It has 2595176 observations and 7 variables. The key variables are `id` and `date`. The `id` identifies a weather station, and the `date` identifies a date. Other variables are the max and min temperature, percipitation, snowfall and snow depth on a specific day from 1981 to 2010 at a specific weather station. There are a lot of data missing. Percipitation data has 94.4% data missing, snowfall data has 85.3% data missing, snow depth data has 77.2% data missing, both tmax and tmin data have 56.3% data missing. It is stated in the NOAA data page that some weather station only collect a subset of these variable, so there are extensive missing data.
